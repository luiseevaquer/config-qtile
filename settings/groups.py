...
for i in groups_dict.keys():
    if i == 'command':
        groups.append(Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'],
            ...
            'stterm -e /home/lescobarvx/.config/qtile/pping.sh google.com wlp2s0 WIFI 1 0',
            'stterm -e /home/lescobarvx/.config/qtile/pping.sh google.com eno1 LAN 2 0',
        ...
        groups.append(Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'],  
            matches=[Match(title=['firefox', 'Firefox', 'Firefox Developer Edition', 'Free Download Manager'], wm_class=['firefox', 'Firefox', 'Firefox Developer Edition', 'TorBrowser', 'google-chrome', 'Google-chrome', 'epiphany', 'epiphany-browser', 'Epiphany', 'Free Download Manager', 'freedownloadmanager'])]))
    elif i == 'terminal':
        groups.append(Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'], 
            matches=[Match(wm_class=['terminator', 'Terminator', 'stterm', 'URxvt', 'wezterm-gui', 'wezterm'])]))
    elif i == 'code':
        groups.append(Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'], 
            matches=[Match(wm_class=['code', 'VSCode', 'vscode', 'geany', 'Geany', 'gedit'])]))
    elif i == 'explorer':
        groups.append(Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'], 
            matches=[Match(title=['nautilus', 'Nautilus', 'thunar', 'Thunar', 'sunflower', 'Sunflower'], 
                wm_class=['nautilus','Nautilus' 'thunar', 'Thunar', 'sunflower', 'Sunflower', 'gnome-calculator', 'Calculadora', 'Calculator'])]))
    #Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'], 
    #   matches=[Match(wm_class=['ulauncher'])])
    elif i == 'multimedia':
        groups.append(Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'],
            matches=[
                Match(title=['Spotify', 'Kodi', 'emby-theater', 'VNC Viewer', 'vncviewer']),
                Match(wm_class=['zoom', 'kodi-bin', 'kodi', 'emby-theater', 'Totem', 'spotify', '/usr/share/spotify/spotify', 'vncviewer']),
            ]))
    elif i == 'tools':
        groups.append(Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'],
            matches=[Match(wm_class=['Shutter', 'pavucontrol', 'Control de Volumen', 'blueman-manager', 'gnome-control-center'])]))
    elif i == 'stack':
        groups.append(Group(i, label=groups_dict[i]['label'], position=groups_dict[i]['pos'],
            matches=[
        ...
        groups.append(ScratchPad(i, [
                ...
                DropDown("switch_wlan", "stterm -e /home/lescobarvx/.config/qtile/switch_wlan.sh", on_focus_lost_hide=False),
...
keys.extend([
    ...
    Key([mod, "mod1"], "XF86Launch7", lazy.group['scratchpad'].dropdown_toggle('switch_wlan')),
    ...
    Key([mod], "XF86Tools", lazy.spawn('/usr/share/spotify/spotify --username=luiseev1078 --password=Shaill2121 --mu=luiseev')),
    Key([], "XF86Tools", lazy.spawn('/usr/share/spotify/spotify --username=m3puxjqyscjurfh8yqjgbalgs --password=Shaill2121 --mu=kcsystems')),
